...
import { DatabaseWorkspaceManager, type DatabaseWorkspaceManagerOptions } from './database-workspace-manager.js';
...
export class AutoWorkspaceManager implements WorkspaceManager {
  ...
  private determineStorageType(): 'file' | 'database' {
    ...
    if (hasPostgresUrl || hasMysqlUrl || isVercel || (isProduction && (hasPostgresUrl || hasMysqlUrl))) {
      return 'database';
    }
