...
import { TimeSeriesDataPoint, TimeSeriesRequest } from '../../types/index.js';
 ...
 */
export function generateTimeSeriesSQL(
  ...
  return `
    WITH ${dialect === 'sqlite' ? 'RECURSIVE ' : ''}${dateSeriesSQL}, daily_stats AS (
      SELECT
      DATE (created_at) as created_date, COUNT (*) as daily_created
      FROM devlog_entries
      WHERE DATE (created_at) BETWEEN ${params}
      GROUP BY DATE (created_at)
      ), daily_closed AS (
      SELECT
      DATE (closed_at) as closed_date, COUNT (*) as daily_closed
      FROM devlog_entries
      WHERE DATE (closed_at) BETWEEN ${closedParams} AND closed_at IS NOT NULL
      GROUP BY DATE (closed_at)
      ), cumulative_stats AS (
      SELECT
      ds.date, COALESCE (dc.daily_created, 0) as daily_created, COALESCE (closed.daily_closed, 0) as daily_closed, (SELECT COUNT (*) FROM devlog_entries WHERE DATE (created_at) <= ds.date) as total_created, (SELECT COUNT (*) FROM devlog_entries WHERE DATE (closed_at) <= ds.date AND closed_at IS NOT NULL) as total_closed
      FROM date_series ds
 ...
 */
export function generateTimeSeriesParams(request: TimeSeriesRequest = {}): string[] {
  const { startDate, endDate } = generateDateRangeToDates(request);
 ...
 */
export function mapSQLRowsToDataPoints(
  rows: Array<{
    date: string;
...
export function generateDateRangeToDates(request: TimeSeriesRequest = {}): {
  startDate: Date;
  endDate: Date;
} {
