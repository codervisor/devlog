...
export function calculateTimeSeriesStats(
  ...
  while (currentDate <= endDate) {
    ...
    const dailyClosed = allDevlogs.filter((devlog) => {
      ...
      if (!devlog.closedAt || (devlog.status !== 'done' && devlog.status !== 'cancelled'))
        return false;
    ...
    const totalCreated = allDevlogs.filter(
      (devlog) => new Date(devlog.createdAt) <= endOfDay,
    ).length;

    const totalClosed = allDevlogs.filter(
      (devlog) => devlog.closedAt && new Date(devlog.closedAt) <= endOfDay,
    ).length;
