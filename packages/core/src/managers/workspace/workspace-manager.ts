...
export class FileWorkspaceManager implements WorkspaceManager {
  ...
  async createWorkspace(
    ...
    if (
      ...
      workspaceCount >= config.globalSettings.maxWorkspaces
    ) {
      throw new Error(
        `Maximum number of workspaces (${config.globalSettings.maxWorkspaces}) reached`,
      );
    }
    ...
    if (config.globalSettings?.namingPattern) {
      ...
      if (!pattern.test(workspace.id)) {
        throw new Error(
          `Workspace ID '${workspace.id}' does not match required pattern: ${config.globalSettings.namingPattern}`,
        );
      }
    }
