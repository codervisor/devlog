...
import { createDataSource, type TypeORMStorageOptions } from '@/storage';
...
export class DatabaseWorkspaceManager implements WorkspaceManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        `Failed to initialize DatabaseWorkspaceManager: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
