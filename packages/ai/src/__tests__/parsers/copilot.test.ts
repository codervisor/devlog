...
describe('CopilotParser Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      if (!hasVSCodeData) {
        console.warn(
          'No VS Code installations with Copilot data found. Some tests will be skipped.',
        );
      }
    } catch (error) {
  ...
  describe('parseChatSession', () => {
    ...
    it('should return null for non-existent session', async () => {
      const session = await parser.parseChatSession(
        'invalid-app',
        'invalid-workspace',
        'invalid-session',
      );
  ...
  describe('Chat message extraction', () => {
    it.runIf(() => hasVSCodeData)('should extract messages with correct structure', async () => {
      ...
      const sessionWithMessages = sessions.find((s) => s.turns.some((t) => t.messages.length > 0));
  ...
  describe('Performance and scalability', () => {
    it.runIf(() => hasVSCodeData)(
      'should handle large numbers of sessions efficiently',
      async () => {
        ...
        console.log(
          `Processed ${totalSessions} sessions from ${workspaces.length} workspaces in ${duration}ms`,
        );
