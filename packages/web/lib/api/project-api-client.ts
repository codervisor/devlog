...
export class ProjectApiClient {
  ...
  async list(): Promise<Project[]> {
    try {
      ...
      throw new ApiError('PROJECT_LIST_FAILED', 'Failed to fetch projects list', 500, {
        originalError: error,
      });
    }
  }
  ...
  async get(projectName: string): Promise<Project> {
    if (!projectName || typeof projectName !== 'string') {
      throw new ApiError('INVALID_PROJECT_NAME', 'Project name must be a non-empty string', 400);
    }
    ...
    try {
      ...
      if (error instanceof ApiError) {
        ...
        if (error.isNotFound()) {
          throw new ApiError('PROJECT_NOT_FOUND', `Project '${projectName}' not found`, 404, {
            projectName,
          });
        }
        throw error;
      }
      throw new ApiError('PROJECT_GET_FAILED', `Failed to fetch project '${projectName}'`, 500, {
        projectName,
        originalError: error,
      });
    }
  ...
  async create(projectData: CreateProjectRequest): Promise<Project> {
    if (!projectData || !projectData.name) {
      throw new ApiError('INVALID_PROJECT_DATA', 'Project name is required', 400, {
    ...
    try {
      ...
      if (error instanceof ApiError) {
        ...
        if (error.isValidation()) {
          throw new ApiError('PROJECT_VALIDATION_FAILED', error.message, 422, {
            ...error.details,
            projectData,
          });
        }
      ...
      throw new ApiError('PROJECT_CREATE_FAILED', 'Failed to create project', 500, {
        projectData,
        originalError: error,
      });
    }
...
export function isProjectApiError(error: unknown): error is ApiError {
  return (
    ...
    (error.code.startsWith('PROJECT_') || error.code.includes('PROJECT'))
  );
}
