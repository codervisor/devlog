 ...
 */
export const BatchEventsCreateSchema = z
  .array(AgentEventCreateSchema)
  .max(1000, 'Cannot create more than 1000 events at once');
...
export const ProjectResolveSchema = z.object({
  repoUrl: z
    ...
    .or(
      z
        .string()
        .regex(/^git@github\.com:.+\/.+\.git$/, 'Repository URL must be a valid GitHub URL'),
    ),
});
