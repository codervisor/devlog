...
import { ChatSessionData, MessageData, WorkspaceDataContainer } from '../models/index.js';
export class CopilotParser {
    logger = console; // TODO: Use proper logger
    constructor() {
        // Initialize parser
    }
    /**
     ...
     */
    async buildWorkspaceMapping(basePath) {
     ...
     */
    async parseChatSession(filePath) {
        try {
     ...
     */
    async parseChatEditingSession(filePath) {
        try {
            const fileContent = await readFile(filePath, 'utf-8');
            const data = JSON.parse(fileContent);
    ...
    getVSCodePaths() {
        ...
        switch (platform()) {
            case 'win32': // Windows
                ...
                paths.push(resolve(appDataRoaming, 'Code', 'User'), resolve(appDataRoaming, 'Code - Insiders', 'User'));
                break;
            case 'darwin': // macOS
                const applicationSupport = resolve(home, 'Library', 'Application Support');
                paths.push(resolve(applicationSupport, 'Code', 'User'), resolve(applicationSupport, 'Code - Insiders', 'User'));
                break;
     ...
     */
    async discoverVSCodeCopilotData() {
                    ...
                    for (const [key, value] of Object.entries(data.metadata)) {
                        if (key in allData.metadata) {
                            if (Array.isArray(allData.metadata[key]) && Array.isArray(value)) {
                                allData.metadata[key].push(...value);
                            }
                            else {
     ...
     */
    async discoverCopilotData(basePath) {
     ...
     */
    searchChatContent(workspaceData, query, caseSensitive = false) {
        const results = [];
     ...
     */
    getChatStatistics(workspaceData) {
        const stats = {
